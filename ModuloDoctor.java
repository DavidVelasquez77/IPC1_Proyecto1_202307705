...
public class ModuloDoctor extends JFrame {
    ...
    public ModuloDoctor(Doctor doctor){
        ModuloDoctor.doctor = doctor;       
        
    }
    public void initialize(){
        Doctor doctorActual = null;
        for (Doctor doctor : Main.doctores) {
            if (doctor.getCodigo().equals(InterfazInicioSesion.cajaCodigo.getText())){
                doctorActual = doctor;
                break;
            }
        }
        if(doctorActual != null){
            String nombreDoctor = doctorActual.getNombres();
            ModuloDoctor.doctor = doctorActual;
            setTitle("Doctor");
            setSize(1000, 1000);
            ...
            editarPerfilDoctor.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    InterfazActualizarDoctor actualizarDoctorFrame = new InterfazActualizarDoctor(finalDoctorActual);
            ...
            btnAtender.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (selectedRow != -1) {
                        ModuloPaciente.setEstadoCita(selectedRow, "Aceptada");
            ...
            btnRechazar.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    int selectedRow = tablaCitas.getSelectedRow();
                    if (selectedRow != -1) {
                        ModuloPaciente.setEstadoCita(selectedRow, "Rechazada");
            ...
            asignarButton.addActionListener(e -> {
                ...
                Doctor doctor = ModuloDoctor.doctor;
                codigoCitas++;
    ...
    public static void main(String[] args) {
        // Busca el doctor correcto antes de crear ModuloDoctor
        Doctor doctor = null;
        for (Doctor d : Main.doctores) {
            if (d.getCodigo().equals(InterfazInicioSesion.cajaCodigo.getText())){
                doctor = d;
                break;
            }
        }
        if (doctor != null) {
            ModuloDoctor doctorFrame = new ModuloDoctor(doctor);
    ...
    public static List<String> getHorarios() {
        ...
        throw new UnsupportedOperationException("Unimplemented method 'getHorarios'");
    }

    public static void setHorarios(List<String> horarios) {
        ModuloDoctor.horarios = horarios;
    }
}